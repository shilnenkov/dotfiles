...
install_ubuntu() {
    ...
    rm -rf /tmp/i3status-rust
    git clone https://github.com/greshake/i3status-rust --branch v0.20.4 /tmp/i3status-rust
    cd /tmp/i3status-rust && cargo install --path . && ./install.sh
